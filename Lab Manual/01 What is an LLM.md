...
At its core, an LLM model is a large file of learned numerical [[10 Glossary#Weights|weights]], not code in the traditional sense. Those weights encode statistical relationships between [[10 Glossary#Token|tokens]] (chunks of text) learned from enormous amounts of training data. Given some input text, the model's entire job is to calculate the probability of what token comes next, then repeat that process one token at a time. Everything else you experience when using an LLM, chat behavior, tool use, coding assistance, is built on top of that one core prediction loop.
...
- **"Bigger" just means more numbers.** That is the [[10 Glossary#Parameters|parameter count]] you see stamped on model names, and it is the next thing we will unpack.

### Model Types

Not every model does the same job out of the box. The differences come from what was done to a [[10 Glossary#Base Model (Foundation Model)|base model]] after that raw prediction training. Here are the categories you will actually run into, and what each is tuned for. Nearly everything you pull for this lab is an [[10 Glossary#Instruct Model|instruct]] or [[10 Glossary#Chat Model|chat]] model; the rest are here so you recognize them in the wild.
...
Model tokens are the linguistic units described above, what the model actually reads and generates internally. A model's [[10 Glossary#Context Window|context window]], how much conversation it can hold onto at once, is measured in these tokens, not words or characters.
